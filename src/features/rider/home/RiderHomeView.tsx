...
export const RiderHomeView: React.FC = () => {
    ...
    const nextShift = useMemo(() => {
        return myShifts
            .filter(s => new Date(s.startAt) > currentTime)
    ...
    const weeklyStats = useMemo(() => {
        ...
        const totalHours = weeksShifts.reduce((acc, s) => acc + (new Date(s.endAt).getTime() - new Date(s.startAt).getTime()) / (1000 * 60 * 60), 0);
    ...
    return (
        <div className={`flex flex-col gap-8 transition-transform pb-20 ${isShaking ? 'animate-haptic' : ''}`}>
            ...
            <div className="grid grid-cols-6 grid-rows-2 gap-4 h-[440px]">
                ...
                <div className="col-span-2 glass-premium rounded-[2rem] p-5 flex flex-col justify-between group hover:scale-[1.02] transition-transform">
                    ...
                    <div>
                        ...
                        <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-1">Sevilla</p>
                    </div>
                </div>
            ...
            <VehicleChecklistModal
                ...
                onSubmit={async (data) => {
                    if (!user?.uid) return;
                    await riderService.submitChecklist(user.uid, { ...data, vehicleId: '' });
                }}
            />
        </div>
    );
