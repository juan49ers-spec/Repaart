...
import { X, BookOpen, ChevronRight, Activity, Smartphone } from 'lucide-react';
...
const FLYDER_GUIDE_ID = 'flyder-ios';
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-start justify-center p-4 pt-16 md:pt-24 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden flex h-[700px] max-h-[90vh]">
                ...
                <div className="w-72 bg-slate-50 dark:bg-slate-900/50 border-r border-slate-100 dark:border-slate-800 flex flex-col backdrop-blur-xl">
                    ...
                    <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="pt-2 pb-1 px-2">
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2 px-2">Guías Interactivas</p>
                        </div>

                        {/* Flyder iOS Guide */}
                        <button
                            onClick={() => setActiveTopicId(FLYDER_GUIDE_ID)}
                            className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl text-sm font-medium transition-all duration-200 group ${activeTopicId === FLYDER_GUIDE_ID
                                ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-md shadow-slate-100 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-700'
                                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-200'
                            }`}
                        >
                            <div className="flex items-center gap-3">
                                <div className={`transition-colors ${activeTopicId === FLYDER_GUIDE_ID ? 'text-indigo-500' : 'text-slate-400 group-hover:text-slate-600'}`}>
                                    <Smartphone className="w-4 h-4" />
                                </div>
                                <span className="truncate">Instalar Flyder (iOS)</span>
                            </div>
                            {activeTopicId === FLYDER_GUIDE_ID && (
                                <ChevronRight className="w-3.5 h-3.5 text-indigo-500 animate-in fade-in slide-in-from-left-2" />
                            )}
                        </button>
                    </nav>
                ...
                {/* Main Content Area */}
                <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-950/50 relative overflow-hidden">

                    {activeTopicId === FLYDER_GUIDE_ID ? (
                        /* Interactive Flyder iOS Guide */
                        <div className="flex-1 overflow-hidden relative">
                            <button
                                onClick={onClose}
                                title="Cerrar"
                                className="absolute top-3 right-3 z-20 p-2 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-full text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-all shadow-sm border border-slate-100 dark:border-slate-800"
                            >
                                <X className="w-4 h-4" />
                            </button>
                            <FlyderInstallGuide />
