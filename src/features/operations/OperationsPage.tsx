        ...
        <div className="h-full bg-slate-50 p-6 overflow-hidden flex flex-col space-y-2">
            ...
            <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 border-b border-slate-200 pb-2 shrink-0">
                ...
                <h1 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                    {activeTab === 'scheduler' && <Calendar className="w-5 h-5 md:w-6 md:h-6 text-indigo-600" />}
                    {activeTab === 'riders' && <Users className="w-5 h-5 md:w-6 md:h-6 text-blue-600" />}
                    {activeTab === 'motos' && <Bike className="w-5 h-5 md:w-6 md:h-6 text-orange-600" />}

                    {activeTab === 'scheduler' ? 'Planificador' :
                        activeTab === 'riders' ? 'Talentos' : 'Flota'}
                    </h1>
