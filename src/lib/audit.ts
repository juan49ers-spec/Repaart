...
export const AUDIT_ACTIONS = {
    ...
    INVOICE_RECTIFICATION: 'INVOICE_RECTIFICATION',
...
export const logAction = async (user: AuditUser | null, action: AuditActionType, details: LogDetails = {}): Promise<void> => {
    ...
    while (retries > 0) {
        try {
            ...
            retries--;
            if (retries > 0) {
                console.warn(`Audit: Failed to log ${action}. Retrying... (${retries} attempts left)`);
                await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1s before retry
            } else {
                console.error("Audit Critical Error: Failed all retries to log action", action, error);

                // SECURITY HARDENING: Block critical actions if audit fails after retries
